...
const View = () => {
    ...
    const fetchData = async () => {
        ...
        let studentProgramResponse = await getApi('api/contact/studentprograms/',param.id)
        let studentOrderdResponse = await getApi('api/contact/orders/',param.id)
        let tasksData = await getApi('api/contact/tasks/',param.id)
        setData(response.data);
        setStudentPrograms(studentProgramResponse.data);
        setStudentOrders(studentOrderdResponse.data);
        setStudentTasks(tasksData.data)
        setIsLoding(false)
    }
    useEffect(() => {
        fetchData()
                                            ...
                                            <Grid templateColumns={{ base: '1fr' }} gap={4}>
                                                ...
                                                <GridItem colSpan={2}>
                                                    {studentPrograms.length > 0 ? (
                                                        <Box border="1px solid" borderColor="gray.200" borderRadius="md" bg="gray.50" p={3}>
                                                            {studentPrograms.map((program) => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <Text fontSize="sm" color={program.isActive ? 'green.600' : 'red.600'}>
                                                                        Active: {program.isActive ? 'Yes' : 'No'}
                                                                    </Text>
                                                                </Box>
                                                            ))}
                                                        </Box>
